...
parser.add_argument("--criterion_to_use", default="softmax_only", help="Criterion to use. Can be 'kendall_and_gal', 'kyles_version' or 'softmax_only'")
...
criterion_dict = {
  ...
  "criterion_kyles_variance": loss_functions.BayesianCategoricalCrossEntropy_KylesVersion(T=100, num_classes=num_classes),
  # For softmax network output in Kyles version
  "criterion_kyles_softmax": nn.CrossEntropyLoss(),
}
criterion_to_use = args.criterion_to_use # "kendall_and_gal" or "kyles_version" or "softmax_only"
...
logger = TensorBoardLogger(
  save_dir=os.getcwd(),
  name="lightning_logs",
  version=dataset_name + "_" + criterion_to_use + "_freeze_encoder-" + str(freeze_encoder_params) + "_" + time
)
